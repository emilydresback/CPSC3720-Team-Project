...
describe('Microservices Integration Tests', () => {
  ...
  describe('Concurrent Booking Scenarios', () => {
    ...
    test('should handle multiple concurrent bookings correctly', async () => {
      ...
      const bookings = await Promise.all([
        request(clientApp).post('/api/bookings').send({ userId: 1, eventId, quantity: 3 }),
        request(clientApp).post('/api/bookings').send({ userId: 2, eventId, quantity: 4 }),
        request(clientApp).post('/api/bookings').send({ userId: 3, eventId, quantity: 2 })
      ]);
      
      // All should succeed
      expect(bookings[0].status).toBe(201);
      expect(bookings[1].status).toBe(201);
      expect(bookings[2].status).toBe(201);
      
      // Verify final ticket count
      const eventResponse = await request(adminApp)
        .get(`/api/events/${eventId}`);
      
      expect(eventResponse.body.availableTickets).toBe(1); // 10 - 3 - 4 - 2 = 1
    });
    
    test('should prevent overbooking in concurrent scenarios', async () => {
      // Create event with very limited tickets
      const createResponse = await request(adminApp)
        .post('/api/events')
        .send({
          name: 'Very Limited Event',
          date: '2025-12-20',
          location: 'Tiny Venue',
          totalTickets: 5,
          price: 25.00
      ...
      const bookings = await Promise.all([
        request(clientApp).post('/api/bookings').send({ userId: 1, eventId, quantity: 3 }),
        request(clientApp).post('/api/bookings').send({ userId: 2, eventId, quantity: 3 })
      ]);
      
      // One should succeed, one should fail
      const successes = bookings.filter(b => b.status === 201).length;
      const failures = bookings.filter(b => b.status === 400).length;
